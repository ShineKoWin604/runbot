...
# --- Main Menu ပြသခြင်း (ဗီဒီယိုထဲက စာသားဒီဇိုင်းအတိုင်း) ---
...
def handle_start(message):
    ...
    # Join Channel စစ်ဆေးမှု မရှိတော့ဘဲ တိုက်ရိုက် Main Menu ပြသမည်
...
# --- Inline Keyboard Callbacks (ခလုတ်များ နှိပ်သည့်လုပ်ဆောင်ချက်) ---
@bot.callback_query_handler(func=lambda call: True)
def handle_callbacks(call):
    filename = call.data.replace("view_", "").replace("stop_", "").replace("delete_", "").replace("logs_", "")
    ...
    if call.data.startswith("view_"):
        ...
        # ဗီဒီယိုထဲက ခလုတ်နေရာအတိုင်း စီစဉ်ထားသည်
        ...
        # ဗီဒီယိုထဲကအတိုင်း Logs များ ဖတ်ပြသသည့်အပိုင်း
        ...
        if filename in log_store and log_store[filename]:
            formatted_log = f"📝 Logs for <u>{filename}</u>:\n\n<code>{log_store[filename]}</code>"
        else:
...
def handle_back_to_files(message, is_callback=False):
    ...
    for filename in file_store:
        # ဗီဒီယိုထဲကအတိုင်း Run နေလျှင် အစိမ်းလုံး၊ ရပ်နေလျှင် အနီလုံး အီမိုဂျီပုံစံ
...
# --- ဖိုင်လက်ခံပြီး နောက်ကွယ်တွင် Auto Run ပေးခြင်း ---
...
def handle_docs(message):
    ...
    if filename.endswith('.py') or filename.endswith('.html'):
        ...
        # ဗီဒီယိုထဲက စာသားပုံစံအတိုင်း ပြန်လည်ဖြေကြားခြင်း
        bot.reply_to(message, f"✅ {filename} ကို လက်ခံရရှိပါပြီ။ စတင် Run နိုင်ပါပြီ။")
        
        # Background Multi-Process စနစ်ဖြင့် စတင် Run ခြင်း
        try:
